...
import { Siren, Radio, Phone, MapPin } from "lucide-react";
  ...
  const handleBroadcast = async () => {
    ...
    try {
      await api.post("/emergency", {
        requesting_hospital_id: "00000000-0000-0000-0000-000000000001",
    ...
    <div className="relative min-h-[calc(100vh-5rem)] -m-6 overflow-hidden rounded-none bg-gradient-to-br from-slate-900 via-slate-800 to-rose-950 p-8">
      ...
      <div className="relative z-10 mx-auto max-w-2xl space-y-8">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold text-white">Emergency Blood Request</h1>
          <p className="mt-2 text-sm text-slate-400">Broadcast urgent requests to all network hospitals</p>
        </div>
          ...
          {broadcastSent ? (
            ...
            >
              <div className="flex items-center justify-center gap-2 text-emerald-400">
                ...
                <span className="text-sm font-medium">Broadcast active — waiting for responses</span>
              </div>
            </motion.div>
          ) : (
            <GradientButton
        ...
        {broadcastSent && (
          ...
          >
            <GlassCard className="border-white/10 bg-white/5 backdrop-blur-xl p-5" hover={false}>
              <h2 className="mb-3 text-sm font-semibold text-white">Live Response Feed</h2>
                ...
                {mockResponses.map((resp, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`h-2 w-2 rounded-full ${resp.status === "available" ? "bg-emerald-400" : "bg-slate-500"}`} />
                      <div>
                        <p className="text-sm font-medium text-white">{resp.hospital}</p>
                        <p className="text-[10px] text-slate-400">{resp.time}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`text-sm font-semibold ${resp.status === "available" ? "text-emerald-400" : "text-slate-500"}`}>
                        {resp.units > 0 ? `${resp.units} units` : "None"}
                      </span>
                      {resp.units > 0 && (
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => toast.success(`Call initiated to ${resp.hospital}`)}
